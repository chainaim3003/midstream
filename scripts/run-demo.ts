...
function renderEvent(e: SessionEvent): void {
  switch (e.type) {
    ...
    case "quality-assessed":
      // Inline score, then indented reasoning on a new line. The reasoning
      // field is already generated by Gemini on every call (schema-required),
      // so showing it costs nothing extra — we just weren't rendering it.
      process.stdout.write(
        ` [q=${e.report.score.toFixed(2)} avg=${e.rollingAvg.toFixed(2)}]`,
      );
      if (e.report.reasoning) {
        process.stdout.write(`\n     ↳ ${e.report.reasoning}\n   `);
      }
      break;
